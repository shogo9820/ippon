...
function changeVoteMode() {
  socket.emit("setVoteMode", document.getElementById("vote-mode-select").value);
}
...
socket.on("updateState", (state) => {
  document.getElementById("setup-section").style.display =
    state.phase === "setup" ? "block" : "none";
  document.getElementById("recruit-section").style.display =
    state.phase === "recruiting" ? "block" : "none";
  document.getElementById("play-section").style.display =
    state.phase === "playing" ? "block" : "none";
  document.getElementById("ranking-section").style.display =
    state.phase === "ranking" ? "block" : "none";
  ...
  if (state.phase === "playing") {
    if (state.mode === "ippon") {
      ...
      document.getElementById("ippon-current-q").innerText =
        state.currentQuestion || "未設定";

      const ipponSelect = document.getElementById("ippon-question-select");
      if (ipponSelect) {
        const currentVal = ipponSelect.value;
        ipponSelect.innerHTML =
          '<option value="">-- 保存済みお題から選択 --</option>';
        const ipponList = state.ipponQuestions || [];
        ipponList.forEach((item) => {
          const qText = typeof item === "object" ? item.text : item;
      ...
      if (sortedScores.length > 0) {
        allScoresEl.innerHTML = sortedScores
          .map(([name, score], idx) => `${idx + 1}位: ${name} (${score}pt)`)
          .join(" / ");
      } else {
      ...
      document.getElementById("ctrl-current-q").innerText =
        state.currentQuestion || "未選択";
      document.getElementById("ctrl-current-a").innerText =
        state.currentAnswer || "-";
